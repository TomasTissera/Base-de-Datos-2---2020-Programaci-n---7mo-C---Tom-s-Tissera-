...
select distinct rental_duration ,title ,
    description from  film f1 ; 
...
select title,rental_rate ,replacement_cost from film f 
...
SELECT a.address, c2.country, c.city 
    FROM store s, address a, city c, country c2 
...
SELECT f.title, s.first_name 
    FROM inventory i ,film f ,staff s, store s2 
